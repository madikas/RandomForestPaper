...
To make a better assement of the ability of these methods to detect unimportant variables, some noise variables will be added.
...
```{r randomForest importance , warning=FALSE , tidy=TRUE , fig.height= 3 , fig.align="center"}
...
```{r Variable importance for the regression case , tidy=TRUE , fig.height= 3 , fig.align="center"} 
...
```{r Computing variable importance using the vimp() function , fig.height=3 , fig.align="center" , fig.width=10 }
...
```{r calculating variable importance on regression using vimp , fig.height= 3 , fig.align="center" , fig.width = 10}
...
for( i in block_size) {
  dotchart(imp_block_size[[i]][10:14] , main = paste("block size" , as.character(i)) , xlim = c(0 , 0.15))
}
```
...
```{r holdout.vimp function to compare , fig.align="center" , fig.height=3 , fig.width=10}
...
dotchart(imp_hold_class[5:9] , labels = rownames(imp_hold_class) , main = "Classifcation task" , xlim = c(min(imp_hold_class[5:9]) - 0.001 , max(imp_hold_class[5:9] + 0.001) ) )
dotchart(imp_hold_reg[11:15] , labels=rownames(imp_hold_reg) , main = "Regression task" , xlim = c(min(imp_hold_reg[11:15])-0.5 , max(imp_hold_reg[11:15])+ 0.5))
```
...
```{r comparing the ranking of the methods by pair , fig.align="center" , fig.height=3 , fig.width=10}
#Create the ranking
